package com.jeethink.process.leavecountersign.service.impl;
...
import com.jeethink.process.leave.domain.BizLeaveVo;
import com.jeethink.process.leave.mapper.BizLeaveMapper;
import com.jeethink.process.leavecountersign.service.IBizLeaveCounterSignService;
import com.jeethink.process.todoitem.domain.BizTodoItem;
import com.jeethink.process.todoitem.service.IBizTodoItemService;
...
@Service
@Transactional
public class BizLeaveCounterSignServiceImpl implements IBizLeaveCounterSignService {
    @Autowired
...
//                        .singleResult();
    ...
    public List<BizLeaveVo> findTodoTasks(BizLeaveVo leave, String userId) {
        ...
        // 根据当前人的ID查询
        List<Task> todoList = taskService.createTaskQuery().processDefinitionKey("leave-countersign").taskAssignee(userId).list();

        // 根据当前人未签收的任务
        List<Task> unsignedTasks = taskService.createTaskQuery().processDefinitionKey("leave-countersign").taskCandidateUser(userId).list();
    ...
    public void complete(BizLeaveVo leave, Boolean saveEntity, String taskId, Map<String, Object> variables) {
        ...
        // 下一节点处理人待办事项
        bizTodoItemService.insertTodoItem(leave.getInstanceId(), leave, "leaveCounterSign");
    }
    ...
    public List<BizLeaveVo> findDoneTasks(BizLeaveVo bizLeave, String userId) {
        ...
        List<HistoricTaskInstance> list = historyService.createHistoricTaskInstanceQuery()
                .processDefinitionKey("leave-countersign")
                .taskAssignee(userId)
                .finished()
    ...
    @Override
    public ProcessInstance submitApplyByCounterSign(BizLeaveVo entity, String applyUserId, Map<String, Object> variables) {
        ...
        ProcessInstance processInstance = runtimeService // 启动流程时设置业务 key
                .startProcessInstanceByKey("leave-countersign", businessKey, variables);
        String processInstanceId = processInstance.getId();
        entity.setInstanceId(processInstanceId); // 建立双向关系
        bizLeaveMapper.updateBizLeave(entity);

        // 下一节点处理人待办事项
        bizTodoItemService.insertTodoItem(processInstanceId, entity, "leaveCounterSign");
